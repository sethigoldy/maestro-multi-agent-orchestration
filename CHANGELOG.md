...
- **The login-shell snapshot was silently empty where `env -0` does not work.** With an `env` that does not support `-0`, nothing followed the marker and an empty environment was cached without a word, where plain `env` used to work. If `env -0` fails or prints nothing, Maestro now runs the login shell again with plain `env`, reads it line by line as before, and prints one warning to stderr that multi-line values may be cut. Both runs share the `MAESTRO_LOGIN_ENV_TIMEOUT_S` limit.
