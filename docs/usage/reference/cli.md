...
maestro task tail <task-id|number>
...
the top-level options, so `maestro --workspace DIR task 1` runs
`task status 1`. Top-level aliases: `maestro list …`, `maestro status <id> …`.
...
configuration, a configured storage backend that cannot be loaded, or an
explicitly passed workspace that does not exist). Missing
