...
| `tail` | Live-follows one task's event stream over SSE (no polling). Accepts a full task id or a numeric task number; the daemon resolves the number to the task id before the stream starts. An unknown reference exits 2 with a message instead of waiting. `--all` follows the global stream of every task and takes no task reference; with `--all` the exit code is always 0 (absent Ctrl-C). Giving neither a reference nor `--all`, or giving both, exits 2 |
