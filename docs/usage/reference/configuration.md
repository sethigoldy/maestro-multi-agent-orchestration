...
| `MAESTRO_LOGIN_ENV` | `1` | Set `0` to stop passing a login-shell environment snapshot (`$SHELL -lc 'env -0'`) to spawned agents. Values that span several lines, such as a PEM key, are kept whole, and anything the profile prints before the variables is ignored. Values are passed to agents byte for byte, even when they are not valid UTF-8. The login shell runs once and prints both an `env -0` listing and a plain `env` listing. If `env -0` fails or prints nothing (an `env` without `-0`), Maestro reads the plain listing line by line and prints a warning to stderr, because a value that spans several lines is then cut to its first line. If nothing can be captured (the shell is missing, times out, does not run the command, as with `/usr/bin/false` or `/sbin/nologin`, or prints neither listing), Maestro prints a warning that the login environment could not be captured and agents get only the daemon's own environment |
| `MAESTRO_LOGIN_ENV_TIMEOUT_S` | `10` | Max seconds to wait for the login shell before falling back to the daemon's own environment. The shell runs only once, so the whole limit is available to a slow profile |
